...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---|---|---|
| 低阻区网格过细电流失稳（§4.6） | 低阻区减密（调整 r/ratio 或 half_mesh） | 转 mesh-quality 深入检查网格分布 |
| 总电流极小时用电流偏置 KCL 违反（§4.1） | 改电压偏置扫到内建电压 80-90% 再切电流 | 转 bias-strategy 核对偏置策略 |
| bandgap reduction 改写的 IV 段不可用（§4.12） | 只取恢复带隙后的最后一段 IV | 需要完整 IV 时改用辅助接触/慢瞬态等不污染 IV 的技巧 |
| 绝缘宏下隧穿/碰撞电离失效（§4.10.3） | 绝缘层换宽禁带半导体宏（sio2 → s-sio2） | 仍无隧穿：检查隧穿模型开关 |
| PICS3D 阈值附近发散 | 检查 auto_finish=rtgain 是否前置完成模式初始化 | 转 pics3d-laser-workflow 三步偏置排查 |
| 高阻/电流阻挡结构低偏不收敛 | newton_par change_variable=yes | 依次尝试慢瞬态/辅助接触/易解器件技巧 |
