...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---|---|---|
| 过粗网格在剧变区欠采样（界面/QW/光模峰，§4.5） | 局部加密：减小 spacing / 加大该层 n / put_mesh ratio | regrid 按材料参数变化自动加密，重跑 .mplt 确认采样 |
| 过细网格在低阻区电流失稳（§4.6） | 低阻区减密：调整 r/ratio 或 half_mesh | 确认不是材料/接触设置问题；仍失稳转 convergence-debugging |
| 全器件均匀加密浪费内存时间（§3.4.3） | 只加密剧变区（局部 double_mesh），不全局加密 | 检查是否误把 double_mesh 当全局加密用 |
| 加密后关键量仍大幅变化（网格未收敛） | 继续加密并重跑关键量对比 | 核对剧变区定位是否遗漏（界面/接触/电流拥挤）；仍无改善检查结构定义 |
| 怀疑网格但实为结构/偏置问题 | 先核对 .layer 层序/材料/接触与偏置策略 | 按 convergence-debugging 全流程排查 |
