...
| 步骤 | 输入 | 输出 | 判停/完成标志 |
|---|---|---|---|
| 1 判断偏置类型 | 器件阻性/工作状态 | 偏置变量（voltage/current）+ 理由 | 用户确认偏置选择 |
| 2 编排正偏序列 | 内建电压/目标电流 | equilibrium/scan/auto_finish 语句 | 各段终止条件明确、KCL 校验通过 |
| 3 多电极控制 | 电极编号/目标电流 | 多变量 scan 语句 + 符号约定 | 各电极电流符合物理预期 |

1. **判断偏置类型**
   - 高阻器件（反向/OLED/宽禁带/大接触电阻/太阳能）→ 电压偏置；低阻/激射 → 电流偏置。
   - 完成标准: 明确写出所选偏置变量与理由。

   🔴 CHECKPOINT · 🛑 STOP：把偏置变量选择（voltage_1 / current_1）与理由告诉用户确认后再继续——选错偏置类型是阈值附近发散的首要原因。
...
2. **编排正偏序列**
   ...
   - 示例: `equilibrium` → `scan var=voltage_1 value_to=-2 init_step=1e-3 max_step=0.1 auto_finish=current_1 auto_until=1e-3 auto_within=5e-4`（gaas10.sol）→ KCL 校验 → `scan var=current_1 value_to=20. print_step=2.`；PICS3D 用 `auto_finish=rtgain auto_until=0.9 auto_within=0.05`。

   🔴 CHECKPOINT · 🛑 STOP：把扫描序列（电压段终止条件/KCL 校验点/电流段目标）给用户确认后再跑；PICS3D 确认 RTG 初始化已前置。

3. **多电极控制**
   - 共享电极先抬参考地；顶部电极多变量同时电流偏置；检查符号（p 侧为负）。
   - 完成标准: 各电极电流符合物理预期，绘图方向正确。
   - 示例: 共享底电极编号后，`scan var=current_2 value_to=... var2=current_3 value2_to=...` 多变量同时扫描；p 侧电流为负（绘图时按需 x(-1)）。

   🔴 CHECKPOINT · 🛑 STOP：把多电极编号与各电极电流符号（p 侧为负）核对结果给用户确认后再交付。
...
| 触发条件 | 一线修复 | 仍失败兜底 |
|---|---|---|
| 总电流极小时用电流偏置（KCL 违反，§4.1） | 改电压偏置扫到内建电压 80-90% 再切电流 | 仍违反：检查收敛性与扫描步长 |
| 电压远高于开启电压（指数电流不收敛，§4.1） | 电压扫描终止在 80-90% 内建电压，用 auto_finish=current 兜底 | 仍不收敛：减小 max_step 或转 convergence-debugging |
| 激射后用电压偏置（钳位无解，§4.1） | 激光器一律电流偏置，电压只用于阈值前 | 仍失败：确认是否已激射、RTG 初始化是否完成 |
| 总电流极小时电流偏置数值精度不足 | 小电流段用电压偏置 | 仍异常：检查 KCL 与电极符号 |
| 多电极电流流向最短路径 | 共享底电极抬参考地，顶部逐电极独立控制 | 仍串扰：核对电极编号与扫描变量对应 |
