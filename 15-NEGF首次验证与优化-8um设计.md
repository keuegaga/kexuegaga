...
## 5b. CBO 敏感性实验（回答"v1.nnp 要不要校正"）

在 nextnano++ kp 中做了对照实验：

| 情形 | 界面 CBO | 最强同周期跃迁 | λ 对应 |
|---|---|---|---|
| v1.nnp（官方 bandoffset=−0.05057 校准） | 0.782 eV | 156.2 meV（z=1.43） | 7.94 µm |
| 去掉校准（默认数据库） | 0.691 eV | 148.6 meV（z=1.55） | 8.35 µm |

结论：

1. **v1.nnp 本身已经带校准块**（文件末尾 `database{ ... valence_bands{ bandoffset = -0.05057 } }`），无需再"补校准"——它正是我们测到 CBO≈0.78 eV 的原因
2. 但该值是 nextnano 官方为复现 Friedrich 论文而设；**默认数据库的 148.6 meV（8.35 µm）反而与论文 300 K 实验（8.4 µm / 147.6 meV）吻合得更好**
3. 两种带阶下结构都支持强偶极的 8 µm 波段跃迁（z=1.4–1.6），说明 8 µm 结果对 CBO 有 ~8 meV 的敏感性，但不消失

因此：

- 目标 **λ=8.0 µm（155 meV）** → 用 v2（barrier CBO=0.78 eV）
- 目标 **复现 Friedrich 实验 8.4 µm** → 用 **v3**（`qcl8um_design_v3_negf_cbo69.negf`，CBO=0.69 eV）
- v1 NEGF 无增益不能全归因于带阶（默认带阶下 nnp 仍有 148 meV 强跃迁）；需结合 v2/v3 结果继续判断（子带基组、占据、自洽势）
